...
Supersedes: ADR-0005
Amended by: ADR-0008 for the article transition overlay, duration/displacement, and validated opening-handoff pixel visibility and presentation-staging clauses only; static destination DOM ownership remains unchanged
Superseded by:
...
- The requested article body is present in the server-rendered static HTML and its DOM is mounted, laid out, and visible by default without body opacity or delayed mounting. JavaScript, hydration, Motion, and route history are not prerequisites for reading it. ADR-0008 permits one presentation-only exception during a validated full-card opening: after the target pathname commits, a fixed inert opaque underlay may temporarily cover destination pixels until both the route and card motion are ready for handoff. The destination DOM stays mounted, semantic, static, and independently readable; its navigation, history, focus, and scroll behavior never wait for that visual layer.
...
- Intermediate animation and scroll states are part of browser validation; tests must inspect mid-frame pixels and the actual topmost painted layer, so a final URL assertion or DOM visibility assertion alone is insufficient.
