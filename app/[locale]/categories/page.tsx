...
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  ...
  const jsonLd = createTermCollectionJsonLd({
    title: ui[locale].allCategories,
    description: 'Article categories',
    url: absoluteSiteUrl(siteMetadata.siteUrl, localizePath('/categories', locale)),
    locale,
    items: sortedCategories.map((category) => ({
      name: category,
      url: absoluteSiteUrl(siteMetadata.siteUrl, localizePath(`/categories/${category}`, locale)),
      description: `${categoryCounts[category]} articles`,
    })),
  })
