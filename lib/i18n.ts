...
export const ui = {
  zh: {
    ...
    tags: '标签',
    search: '搜索',
    searchDescription: '搜索 mizorewww 博客的全部文章。',
    searchHint: '输入关键词开始搜索。',
    searchLoading: (query: string) => `正在搜索「${query}」。`,
    searchNoResults: (query: string) => `未找到「${query}」相关的文章。`,
    searchUnavailable: (query: string) => `无法搜索「${query}」：搜索索引暂不可用。`,
    searchResultCount: (count: number, query: string) => `找到 ${count} 篇关于「${query}」的文章。`,
    tableOfContents: '目录',
    latest: '最新文章',
    allPosts: '全部文章',
    allCategories: '全部分类',
    ...
    chartLoadError: '图表加载失败，请检查 ECharts 配置。',
    viewCodeOnGithub: '在 GitHub 查看代码',
    viewDiffOnGithub: '在 GitHub 查看 diff',
    toggleTheme: '切换暗色模式',
    ...
    categoryDetailTitle: (category: string) => `分类：${category}`,
    tagDetailTitle: (tag: string) => `#${tag}`,
  ...
  en: {
    ...
    tags: 'Tags',
    search: 'Search',
    searchDescription: 'Search all articles on the mizorewww blog.',
    searchHint: 'Start typing to search.',
    searchLoading: (query: string) => `Searching for "${query}".`,
    searchNoResults: (query: string) => `No results found for "${query}".`,
    searchUnavailable: (query: string) =>
      `Unable to search for "${query}": the search index is unavailable.`,
    searchResultCount: (count: number, query: string) =>
    ...
    copied: 'Copied',
    copyFailed: 'Failed',
    copyCodeFailed: 'Unable to copy. Select the code manually.',
    chartLoadError: 'Failed to load the chart. Check the ECharts option.',
    viewCodeOnGithub: 'View code on GitHub',
    viewDiffOnGithub: 'View diff on GitHub',
    ...
    categoryDetailTitle: (category: string) => `Category: ${category}`,
    tagDetailTitle: (tag: string) => `#${tag}`,
