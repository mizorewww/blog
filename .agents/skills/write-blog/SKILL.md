...
3. **数学与定价**：公式全部用 LaTeX 写出。行内公式使用 `$formula$`（如 `$S_T$`、`$r = 0$`、`$\sigma$`）；
   只有完整数学等式才用 `$$...$$` 单独起段；**严禁**在正文行内使用 `$$...$$` 语法。
   公式与 backtest 代码一一对应，逐个解释符号含义与假设。
   原生支持 TradingView 组件：单行 `$BTC`、`$ETH`、`$SOL` 等常见加密货币代码自动映射为 `BINANCE:${symbol}USDT` 迷你行情走势卡片，`::tv` 嵌入全功能交互图表。
4. **数据与假设**：数据源、窗口、样本量、关键假设（成交价、费用、滑点、币本位机制）。
5. **结果与全量图集**：开头放标准回测性能看板（在 MDX 中调用 `<StrategyCard data={{ ... }} />` 填入纯 JSON 字段即可，无需手写复杂 JSX，详见 `.agents/skills/trading-humanizer/SKILL.md`）。
